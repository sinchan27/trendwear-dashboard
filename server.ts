...
async function startServer() {
  ...
  app.get("/api/demand-validation/:sku_id", (req, res) => {
    try {
      ...
      if (logs.length === 0) {
        // Dynamically initialize 12-week timeline for newly uploaded SKU
        const baseVolume = 800 + (Math.abs(sku_id.split('').reduce((a, c) => a + c.charCodeAt(0), 0)) % 1000);
        for (let w = 1; w <= WEEKS_COUNT; w++) {
          const pred = Math.round(baseVolume * (0.8 + (w * 0.05)) + (Math.sin(w) * 50));
          let actual: number | null = null;
          if (w <= currentWeekReconciled) {
