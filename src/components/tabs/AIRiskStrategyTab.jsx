...
import { BrainCircuit, AlertTriangle, Layers, Send, Building2 } from 'lucide-react';
import { MetricCard } from '../SharedComponents';
import { SUPPLIERS as DEFAULT_SUPPLIERS } from '../../data/constants';

export default function AIRiskStrategyTab({ suppliers = DEFAULT_SUPPLIERS }) {
  const activeSuppliers = suppliers && suppliers.length > 0 ? suppliers : DEFAULT_SUPPLIERS;
  const [formData, setFormData] = useState({ 
    supplierName: activeSuppliers[0]?.name || 'Shantipur Handloom Cluster', 
    leadTime: 5, 
    otdPercent: activeSuppliers[0]?.otd || 85, 
    costPerUnit: activeSuppliers[0]?.cost || 450 
  });
  ...
  const handleSelectSupplier = (e) => {
    ...
    const match = activeSuppliers.find(s => s.name === selectedName);
    if (match) {
      setFormData({
        supplierName: match.name,
        leadTime: match.leadTime || 5,
        otdPercent: match.otd,
        costPerUnit: match.cost
      });
    } else {
      setFormData(prev => ({ ...prev, supplierName: selectedName }));
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <h3 className="text-base font-semibold text-slate-200 mb-4 flex items-center gap-2">
          <Layers size={18} className="text-indigo-400" /> Supplier Strategic Input
        </h3>

        {/* Quick Select from Active Master Data */}
        <div className="mb-4">
          <label className="block text-xs font-medium text-slate-400 mb-1">
            Load From Active Suppliers (Data Hub)
          </label>
          <select
            onChange={handleSelectSupplier}
            value={formData.supplierName}
            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-200 focus:border-blue-500 focus:outline-none"
          >
            {activeSuppliers.map((s) => (
              <option key={s.id || s.name} value={s.name}>
                {s.name} ({s.region}) - OTD {s.otd}%
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="lg:col-span-2 space-y-6">
        {results ? (
          <>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="text-base font-semibold text-slate-200 mb-4 flex items-center gap-2">
                <BrainCircuit size={18} className="text-amber-400" /> Strategic SWOT Analysis ({formData.supplierName})
              </h3>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-10 flex flex-col items-center justify-center text-slate-500 h-full">
            ...
            <p className="text-sm">Input or select a supplier to generate an SVM risk classification and automated SWOT analysis.</p>
          </div>
        )}
      </div>
    </div>
