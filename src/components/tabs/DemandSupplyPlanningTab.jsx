...
export default function DemandSupplyPlanningTab({ demandSurge, capacityLoss }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="space-y-4">
          {SKUS.map((sku) => {
            const adjDemand = Math.round(sku.demand * (1 + demandSurge / 100));
            const adjCap = Math.round(sku.capacity * (1 - capacityLoss / 100));
            const gap = adjCap - adjDemand;
            const safetyPct = Math.min(100, (sku.safetyStock / (adjCap || 1)) * 100);
              ...
              <div key={sku.id} className="bg-slate-950/60 p-4 rounded-lg border border-slate-800/80">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded font-medium ${gap < 0 ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}>
                    {gap < 0 ? `Deficit: ${gap} units` : `Surplus: +${gap} units`}
                  </span>
                </div>
                <div className="space-y-1.5 text-xs text-slate-400">
                  ...
                  <div className="text-[10px] text-amber-300/80">Safety stock threshold: {sku.safetyStock.toLocaleString('en-IN')} units</div>
                </div>
              </div>
            );
          })}
          ...
          {SKUS.map((sku) => (
            <div key={sku.id} className="p-3 bg-slate-950/40 rounded-lg border border-slate-800 text-xs flex justify-between items-center">
              <div>
                ...
                <div className="text-slate-500">Sell-through: {sku.sellThrough}% · {formatINR(sku.price)}</div>
              </div>
              <div className="text-right">
                <span className={`px-2 py-1 rounded text-[10px] font-semibold ${sku.sellThrough < 60 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-slate-800 text-slate-400'}`}>
                  {sku.sellThrough < 60 ? 'Trigger 20% markdown' : 'Hold price'}
                </span>
              </div>
            </div>
          ))}
