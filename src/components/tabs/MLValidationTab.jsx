...
import { MetricCard, NoDataEmptyState } from '../SharedComponents';
import { calculateForecastMetrics } from '../../utils/helpers';

export default function MLValidationTab({ skus = [], isForecastIngested = false, onNavigateDataHub }) {
  ...
  const [activeSubView, setActiveSubView] = useState('operational'); // 'operational' | 'offline_sandbox' | 'python_code'
...
def update_actual_demand(sku_id: str, payload: ActualSalesUpdateRequest, db: Session = Depends(get_db)):
  ...
  if (!isForecastIngested || skus.length === 0) {
    return (
      <div className="space-y-6">
        <NoDataEmptyState 
          title="No Forecast Dataset Ingested for ML Validation"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Visualizing SKU <span className="text-slate-300 font-semibold">{selectedSkuId}</span> ({selectedSkuObj.name}). Dashed line indicates the dynamic Gemini AI demand forecast, solid line represents reconciled actual sales.
                </p>
              </div>
