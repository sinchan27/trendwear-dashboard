...
export default function SupplierRiskTab({ riskAdjusted, setRiskAdjusted, disruptedMillIds, suppliers, setSuppliers, skus, isForecastIngested, onNavigateDataHub }) {
  ...
  const atRiskSkuNames = atRiskSkuIds.map((id) => activeSkus.find((sku) => sku.id === id)?.name).filter(Boolean);
  ...
  if (!isForecastIngested || activeSkus.length === 0) {
    return (
      <div className="space-y-6">
        <NoDataEmptyState 
          title="No Forecast Dataset Ingested for Procurement Engine"
        ...
        <table className="w-full text-left text-xs text-slate-300">
          ...
          <tbody className="divide-y divide-slate-800">
            {suppliers.map((s) => {
              ...
              const allocated = isDisrupted ? 0 : riskAdjusted ? (s.optimizedAlloc !== undefined ? s.optimizedAlloc : s.baseAlloc) : (s.baseAlloc || 0); 
